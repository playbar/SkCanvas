...
void CharacterGame::initialize()
{
    ...
    // Initialize scene.
	_scene->visit((VisitNode*)this, &VisitNode::visitInitNode);
...
{
    if (node->getDrawable() && ((bool)cookie == node->hasTag("transparent")))
...
void CharacterGame::render(float elapsedTime)
{
	...
	_scene->visit(this, &VisitNode::visitDrawNode, (void*)0);
	_scene->visit(this, &VisitNode::visitDrawNode, (void*)1);
