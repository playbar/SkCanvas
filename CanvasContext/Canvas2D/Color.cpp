...
bool Color::parseHexColor(const char* name, unsigned length, RGBA32& rgb)
{
...
bool Color::parseHexColor(const std::string& name, RGBA32& rgb)
{
    ...
    if (!length)
        return false;
	return parseHexColor(name.c_str(), name.length(), rgb);
}
...
bool Color::setFromString(const std::string& name)
{
    ...
    return parseHexColor(name.c_str() + 1, name.length() - 1, m_color);
}

std::string Color::serializedAsCSSComponentValue() const
{
    std::string result;
	result.resize(32);
	SkASSERT(false);
...
std::string Color::nameForRenderTreeAsText() const
{
	SkASSERT(false);
	return std::string("");
    //if (alpha() < 0xFF)
