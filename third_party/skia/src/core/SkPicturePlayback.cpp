    ...
    while (!reader.eof()) {
        ...
        switch (op) {
            ...
            case DRAW_POS_TEXT_TOP_BOTTOM: {
                ...
                size_t points = reader.readInt();
                const SkPoint* pos = (const SkPoint*)reader.skip(points * sizeof(SkPoint));
                const float top = reader.readScalar();
                const float bottom = reader.readScalar();
            ...
            case DRAW_POS_TEXT_H: {
                ...
                const float constY = reader.readScalar();
                const float* xpos = (const float*)reader.skip(xCount * sizeof(float));
            ...
            case DRAW_POS_TEXT_H_TOP_BOTTOM: {
                ...
                getText(reader, &text);
                size_t xCount = reader.readInt();
                const float* xpos = (const float*)reader.skip((3 + xCount) * sizeof(float));
                const float top = *xpos++;
                const float bottom = *xpos++;
                const float constY = *xpos++;
                if (!canvas.quickRejectY(top, bottom)) {
            ...
            case DRAW_TEXT: {
                const SkPaint& paint = *getPaint(reader);
                getText(reader, &text);
                float x = reader.readScalar();
                float y = reader.readScalar();
                canvas.drawText(text.text(), text.length(), x, y, paint);
            } break;
            case DRAW_TEXT_TOP_BOTTOM: {
                const SkPaint& paint = *getPaint(reader);
                getText(reader, &text);
                const float* ptr = (const float*)reader.skip(4 * sizeof(float));
                // ptr[0] == x
                // ptr[1] == y
                // ptr[2] == top
                // ptr[3] == bottom
            ...
            case SCALE: {
                float sx = reader.readScalar();
                float sy = reader.readScalar();
                canvas.scale(sx, sy);
            } break;
            case SET_MATRIX: {
                SkMatrix matrix;
            ...
            case TRANSLATE: {
                float dx = reader.readScalar();
                float dy = reader.readScalar();
